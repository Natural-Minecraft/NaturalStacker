...
public final class McMMO220Hook {
    ...
    public static void register(NaturalStacker plugin) {
        McMMO220Hook.plugin = plugin;
        mcMMO = com.gmail.nossr50.mcMMO.p;
        plugin.getProviders().registerEntityDeathListener(McMMO220Hook::handleDeath);
        plugin.getProviders().registerNameChangeListener(McMMO220Hook::updateCachedName);
    }
    ...
    private static boolean hasTag(LivingEntity livingEntity, MobMetaFlagType mobMetaFlagType) {
        return MobMetadataUtils.hasMobFlag(mobMetaFlagType, livingEntity);
    }
