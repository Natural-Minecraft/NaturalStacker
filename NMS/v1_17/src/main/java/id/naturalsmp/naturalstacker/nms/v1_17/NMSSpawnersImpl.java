package id.naturalsmp.naturalstacker.nms.v1_17;
...
public class NMSSpawnersImpl extends id.naturalsmp.naturalstacker.nms.v1_17.AbstractNMSSpawners {
    ...
    public void registerSpawnConditions() {
        ...
        createCondition("ANIMAL_LIGHT_AND_COLD", (world, position) -> {
            ResourceKey<Biome> biome = world.getBiomeName(position).orElse(null);
            boolean coldBiome = Objects.equals(biome, Biomes.FROZEN_OCEAN) ||
                    Objects.equals(biome, Biomes.DEEP_FROZEN_OCEAN);
        ...
        createCondition("IN_SLIME_CHUNK_OR_SWAMP", (world, position) -> {
            ResourceKey<Biome> biomeBase = world.getBiomeName(position).orElse(null);

            if (Objects.equals(biomeBase, Biomes.SWAMP))
                return true;
        ...
        createCondition("IN_FISH_WATER_OR_LUSH_CAVES",
                (world, position) -> world.getFluidState(position.below()).is(FluidTags.WATER) &&
                        (Objects.equals(world.getBiomeName(position).orElse(null), Biomes.LUSH_CAVES) ||
                                (world.getBlockState(position.above()).is(Blocks.WATER) &&
                                        position.getY() >= world.getSeaLevel() - 13 && position.getY() <= world.getSeaLevel())),
        ...
        createCondition("NOT_IN_OCEAN", (world, position) -> {
            ResourceKey<Biome> biome = world.getBiomeName(position).orElse(null);
            return !Objects.equals(biome, Biomes.OCEAN) &&
                    !Objects.equals(biome, Biomes.DEEP_OCEAN);
        }, EntityType.DOLPHIN);

        createCondition("IN_RIVER", (world, position) -> {
            ResourceKey<Biome> biome = world.getBiomeName(position).orElse(null);
            return !Objects.equals(biome, Biomes.RIVER) &&
                    !Objects.equals(biome, Biomes.FROZEN_RIVER);
        }, EntityType.DOLPHIN);
