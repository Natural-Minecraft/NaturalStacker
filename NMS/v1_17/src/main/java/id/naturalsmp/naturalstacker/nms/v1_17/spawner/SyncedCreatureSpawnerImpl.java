...
public class SyncedCreatureSpawnerImpl extends CraftBlockEntityState<SpawnerBlockEntity> implements SyncedCreatureSpawner {
    ...
    public void setSpawnedItem(org.bukkit.inventory.ItemStack bukkitItem) {
        ...
        CompoundTag blockEntityCompound = baseSpawner.save(this.serverLevel, this.blockPos, new CompoundTag());
        ...
        compound.putInt("Weight", blockEntityCompound.contains("Weight", CraftMagicNumbers.NBT.TAG_ANY_NUMBER) ?
                blockEntityCompound.getInt("Weight") : 1);

        baseSpawner.setNextSpawnData(this.world.getHandle(), this.getPosition(), new SpawnData(entity));
        baseSpawner.spawnPotentials = BaseSpawner.EMPTY_POTENTIALS;
    }
    ...
    BaseSpawner getSpawner() {
        return getBlockEntity().getSpawner();
    }

    SpawnerBlockEntity getBlockEntity() {
        return (SpawnerBlockEntity) this.serverLevel.getBlockEntity(this.blockPos);
    }

    private ResourceLocation getMobName() {
        String id = getSpawner().nextSpawnData.getTag().getString("id");
